...
def scrape_utr_history(df, email, password, offset=0, stop=1, writer=None):
    ...
    for i in range(len(df)):
        ...
        search_url = f"https://app.utrsports.net/search?query={df['f_name'][i+offset]}%20{df['l_name'][i+offset]}&sportTypes=tennis,pickleball&startDate={date_today}&utrMin=1&utrMax=16&utrType=verified&utrTeamType=singles&utrFitPosition=6&type=players&lat=47.16315729999999&lng=-122.0267787"
        ...
        driver.find_element(By.XPATH, '//*[@id="myutr-app-body"]/div/div/div[2]/div/button[6]').click()
        time.sleep(1)
        ...
        try:
            name = soup.find("h1", class_="headerV3__playerName__1mnMa").text
        except:
            name = driver.find_element(By.XPATH, '//*[@id="myutr-app-body"]/div/div[1]/div[1]/div/div/div/div[1]/div[1]/div[2]/h1').text
        ...
        for i in range(len(utrs)):
            ...
            if i == 0:
                continue
            utr = utrs[i].find("div", class_="newStatsTabContent__historyItemRating__GQUXw").text
            utr_date = utrs[i].find("div", class_="newStatsTabContent__historyItemDate__jFJyD").text

            data_row = [name, utr_date, utr]
